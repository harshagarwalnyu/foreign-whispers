...
async def diarize_endpoint(video_id: str):
    ...
    # Return cached result — preserve the skipped flag from the cache so
    # real diarization (skipped=False) is distinguished from the fallback.
    if diar_path.exists():
    ...
    # Step 2: Run diarization (returns [] with a warning when pyannote unavailable)
