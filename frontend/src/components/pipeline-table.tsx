...
const STAGES: {
  ...
  { key: "diarize", label: "Diarize", icon: UsersIcon, description: "Speaker diarization via pyannote.audio" },
...
export function PipelineTable({ pipelineState, settings }: PipelineTableProps) {
  ...
  function getConfig(stage: PipelineStage): string {
    switch (stage) {
      ...
      case "diarize":
        return settings.diarization.length > 0 ? "pyannote/speaker-diarization-3.1" : "skipped";
      case "translate":
